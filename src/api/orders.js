...
import { buildLocalSearchParams, buildOrderListParams, buildSearchFilter } from './backend/helpers'
import {
  buildOrderPayload,
  mapLocalListItem,
  mapOrderListItem,
...
export async function getPopularRestaurants(filters = {}) {
  if (isApiConfigured()) {
    const params = buildLocalSearchParams(filters)
    const qs = params.toString()
    const data = await apiFetchSafe(`/clientes${qs ? `?${qs}` : ''}`)
    if (!data) return []
...
export async function getMyOrders(filters = {}) {
  if (isApiConfigured()) {
    ...
    const qs = params.toString()
    const data = await apiFetch(`/pedidos/clientes/${user.id}${qs ? `?${qs}` : ''}`)
